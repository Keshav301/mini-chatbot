...
async function generateResponse(aiChatBox) {
  ...
  try {
    ...
    if (!data.candidates || !data.candidates[0]) {
      text.innerHTML = `<p style="color:red;">No response or API error.</p>`;
      return;
    }
...
function handlechatResponse(usermessage) {
  ...
  let html = `
    ...
    <div class="user-chat-area">
      ${user.message || "<em>(File only message)</em>"}
      ${
        user.file.data
          ? user.file.mime_type === "application/pdf"
            ? `<div class="pdf-preview">📄 Uploaded PDF</div>`
  ...
  setTimeout(() => {
    let html = `
      ...
      <div class="ai-chat-area">
        <img src="load.webp" alt="Loading..." class="load" width="50px">
      </div>`;
...
imageinput.addEventListener("change", () => {
  ...
  reader.onload = (e) => {
    ...
    if (file.type.startsWith("image/")) {
      ...
      image.src = "pdf-icon.svg"; // use your own PDF icon
    }
